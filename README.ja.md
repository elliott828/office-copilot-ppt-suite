...
1. **企業環境の制約を出発点にする。** Microsoft 365 Copilot は導入されていても、コミュニティ Skills、任意パッケージ、ダウンロードしたコードの直接実行を許可しない組織は少なくありません。実際に使える拡張点は Copilot Agents、承認済み SharePoint ナレッジ、管理されたデスクトップ自動化です。本リポジトリは、制限のない開発環境を前提にせず、この境界に沿って設計されています。
2. **HTML をデザイン面として使う。** 現在のプレゼンテーションおよび Web 生成手法は、HTML と CSS によるレイアウト構成に強みがあります。固定 16:9 キャンバスなら位置と寸法を予測できるため、PowerPoint の構築前に HTML 上で視覚構成を完成させます。
3. **任意の Web ページではなく、制約付き契約をコンパイルする。** PPT-HTML は 960 × 540 座標系と埋め込みの意味的 JSON モデルを持つ制約付き形式です。Compiler は DOM と CSS cascade 全体を逆解析せず、このモデルを読みます。これにより、マッピングを有限、テスト可能、バージョン管理可能にします。
4. **Compiler を固定する。** Agent は依頼ごとに異なるデッキ モデルを生成しますが、VBA 変換エンジンは書き直しません。安定した Compiler は一度まとめてレビュー、署名、回帰テスト、承認でき、変化はデッキ内容側に限定されます。
5. **ネイティブ編集性を保ち、オブジェクト増加を避ける。** PowerPoint object model が対応する場合、一つの意味的な視覚要素を一つの PowerPoint オブジェクトにします。テキスト付きのスタイル コンテナは、背景 shape と別 textbox ではなく、`TextFrame2` を持つ一つの shape になります。表とグラフには再構築データを持たせ、ネイティブ オブジェクトとして残します。
6. **外部 Skills をガバナンス経由で内部化する。** GitHub の上流 Skills は、信頼されていない調査資料として扱います。Curator が出所とライセンスを記録し、有用なパターンを抽出し、互換性テストと人の承認を経て内部標準を公開します。本番 Agents は `Published/Current` のみを参照するため、上流変更が通常のデッキ生成を無断で変えることはありません。
7. **責務を分離する。** オーサリング、キュレーション、コンパイル、QA は失敗形態と更新周期が異なるため、明示的な受け渡しを持つ別々の Agent または能力にします。Compiler はワークフロー上は入れ子ですが、パッケージ上は同列の Skill `$ppt-html-vba-compiler` とし、単独で発見・呼び出せるようにします。
8. **楽観的な主張ではなく証拠を求める。** 未知の Schema メジャー版とオブジェクト型は拒否します。視覚忠実度にはレンダー比較、ネイティブ編集性にはオブジェクト一覧または直接確認、デスクトップ コンパイルには出力ファイルと compile report が必要です。入力を準備しただけの言語モデルが VBA を実行したと主張してはいけません。
